...
class PlatformUITests extends PlatformUI {
    ...
    /**
     * @Given I have a Content Type with identifier :contentIdentifier in Group with identifier :contentGroupIdentifier with fields:
     */
    public function iCreateContentType($contentIdentifier, $contentGroupIdentifier, TableNode $fields) {
        ...
        $repository->sudo(
            function() use($repository, $contentIdentifier, $contentGroupIdentifier, $fields) {
                ...
                $contentTypeGroup = $contentTypeService->loadContentTypeGroupByIdentifier($contentGroupIdentifier);
                $contentTypeGroups = array($contentTypeGroup);

                // Create the structure of the content type based on the $fields values
                $contentTypeCreateStruct = $contentTypeService->newContentTypeCreateStruct(strtolower($contentIdentifier));
                $contentTypeCreateStruct->mainLanguageCode = 'eng-GB';
                $contentTypeCreateStruct->names = array('eng-GB' => $contentIdentifier);
                foreach ($fields as $field) {
